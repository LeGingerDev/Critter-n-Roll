...
namespace ScoredProductions.StreamLinked.LightJson {
	...
	public readonly struct JsonValue
	{
		...
		/// </summary>
		public readonly JsonValueType Type => this.type;

		/// <summary>
		/// Gets a value indicating whether this JsonValue is Null.
		/// </summary>
		public readonly bool IsNull => this.Type == JsonValueType.Null;

		/// <summary>
		/// Gets a value indicating whether this JsonValue is a Boolean.
		/// </summary>
		public readonly bool IsBoolean => this.Type == JsonValueType.Boolean;
		...
		public readonly bool IsInteger
		{
			...
			{
				if (!this.IsNumber) {
					if (this.IsString && int.TryParse((string)this.reference, out _)) {
						return true;
					}
		...
		/// <summary>
		/// Gets a value indicating whether this JsonValue is an Long.
		/// </summary>
		public readonly bool IsLong
		{
			...
			{
				if (!this.IsNumber) {
					if (this.IsString && long.TryParse((string)this.reference, out _)) {
						return true;
					}
				...
				return (value >= long.MinValue) && (value <= long.MaxValue) && unchecked((long)value) == value;
			}
		}
		
		/// <summary>
		/// Gets a value indicating whether this JsonValue is an Float.
		/// </summary>
		public readonly bool IsFloat
		{
			...
			{
				if (!this.IsNumber) {
					if (this.IsString && float.TryParse((string)this.reference, out _)) {
						return true;
					}
				...
				return (value >= float.MinValue) && (value <= float.MaxValue) && unchecked((float)value) == value;
			}
		}
		
		/// <summary>
		/// Gets a value indicating whether this JsonValue is an Byte.
		/// </summary>
		public readonly bool IsByte
		{
			...
			{
				if (!this.IsNumber) {
					if (this.IsString && byte.TryParse((string)this.reference, out _)) {
						return true;
					}
				...
				return (value >= byte.MinValue) && (value <= byte.MaxValue) && unchecked((byte)value) == value;
			}
		}
		...
		/// </summary>
		public readonly bool IsNumber => this.Type == JsonValueType.Number;

		/// <summary>
		/// Gets a value indicating whether this JsonValue is a String.
		/// </summary>
		public readonly bool IsString => this.Type == JsonValueType.String;

		/// <summary>
		/// Gets a value indicating whether this JsonValue is a JsonObject.
		/// </summary>
		public readonly bool IsJsonObject => this.Type == JsonValueType.Object;

		/// <summary>
		/// Gets a value indicating whether this JsonValue is a JsonArray.
		/// </summary>
		public readonly bool IsJsonArray => this.Type == JsonValueType.Array;

		/// <summary>
		/// Gets a value indicating whether this JsonValue represents a DateTime.
		/// </summary>
		public readonly bool IsDateTime => this.AsDateTime.HasValue;

		/// <summary>
		/// Gets all the conditions for the value, used for debugging purposes
		/// </summary>
		public readonly Dictionary<string, bool> GetIsStateConditions => new Dictionary<string, bool>() { 
			{ nameof(this.IsNull), this.IsNull },
			{ nameof(this.IsBoolean), this.IsBoolean },
			{ nameof(this.IsInteger), this.IsInteger },
			{ nameof(this.IsLong), this.IsLong },
			{ nameof(this.IsNumber), this.IsNumber },
			{ nameof(this.IsString), this.IsString },
			{ nameof(this.IsJsonObject), this.IsJsonObject },
			{ nameof(this.IsJsonArray), this.IsJsonArray },
			{ nameof(this.IsDateTime), this.IsDateTime },
		};

		/// <summary>
		/// Gets this value as a Boolean type.
		/// </summary>
		public readonly bool AsBoolean => this.Type switch {
			JsonValueType.Boolean => this.value == 1,
			JsonValueType.Number => this.value != 0,
			JsonValueType.String =>
				bool.TryParse((string)this.reference, out bool result) // true false
					? result 
					: (double.TryParse((string)this.reference, out double number) // numeric 0, 1 ext
						? number != 0
						: !string.IsNullOrEmpty((string)this.reference)),
			_ => false
		};
		...
		public readonly int AsInteger
		{
			get
			{
				if (this.IsNull || !this.IsNumber) {
					if (this.IsString && int.TryParse((string)this.reference, out int result)) {
						return result;
					}
		...
		public readonly long AsLong
		{
			...
			{
				if (this.IsNull || !this.IsNumber) {
					if (this.IsString && long.TryParse((string)this.reference, out long result)) {
						return result;
					}
		...
		/// <summary>
		/// Gets this value as an Float type.
		/// </summary>
		public readonly float AsFloat
		{
			...
			{
				if (this.IsNull || !this.IsNumber) {
					if (this.IsString && float.TryParse((string)this.reference, out float result)) {
						return result;
					}
		...
		public readonly byte AsByte
		{
			...
			{
				if (this.IsNull || !this.IsNumber) {
					if (this.IsString && byte.TryParse((string)this.reference, out byte result)) {
						return result;
					}
			...
			get
			{
				switch (this.Type)
				{
					...
					case JsonValueType.String:
						if (double.TryParse((string)this.reference, out double number))
						{
							return number;
						}
						else
			...
			get
			{
				return this.Type switch {
					JsonValueType.Boolean => (this.value == 1)
												? "true"
												: "false",
					JsonValueType.Number => this.value.ToString(),
